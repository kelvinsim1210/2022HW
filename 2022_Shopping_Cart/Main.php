...
<body>
    ...
    <?php 
        ...
        $result_Commodity_Details = mysqli_query($Shopping_Cart, "SELECT * FROM `commodity_details` WHERE `Commodity_Num`>0 ORDER BY `Commodity_Num`;");
        ...
        for($x=0; $x<count($Datas_Commodity_Details); $x++) {
            ...
            // </td>";
            $Background_Color_Control = "";  // 背景顏色控制
            if($Datas_Commodity_Details[$x]["Commodity_Num"] <= 500) {  // 小於500，背景為綠
                $Background_Color_Control = "green";
            }
            else if($Datas_Commodity_Details[$x]["Commodity_Num"] > 1000) {  // 大於1000，背景為紅
                $Background_Color_Control = "red";
            }
            else {  // 其餘情況，背景為藍
                $Background_Color_Control = "blue";
            }
            $Show_Commodity_Details_Table_innerHTML .= "<tr style='background-color: ".$Background_Color_Control.";'>";
